    ...
    public sealed class Database
    {
       ...
       // public IDbContext DbContext;
        ...
        public Database(IDbContext dbContext)
        {
            ...
            if (DbProvider == null)
            {
                DbProviderFactory = DbContext.GetDbFactory();
                DbProvider = CreateDbProvider(DbContext.DBType, DbContext.ConnectionString);
            }
        }
        ...
        public List<T> ExecuteReader<T>(DbCommand command) where T : class, new()
        {
            DbConnection connection = GetConnection(true);
            PrepareCommand(command, connection);
            ...
            try
            {
                ...
                using (var reader = DoExecuteReader(command, CommandBehavior.CloseConnection))
                {
                    while (reader.Read())
                    {
                        T entityWhile = new T();
                        DbProvider.SetEntityMembers<T>(reader, entityWhile);
        ...
        public T ExecuteReaderToEntity<T>(DbCommand command) where T : class, new()
        {
            DbConnection connection = GetConnection(true);
            PrepareCommand(command, connection);
            ...
            try
            {
                ...
                using (var reader = DoExecuteReader(command, CommandBehavior.CloseConnection))
                {
                    ...
                    {
                        T entity = new T();
                        DbProvider.SetEntityMembers<T>(reader, entity);
                        return entity;
                    }
                    else
                    {
